...
def evaluate_metrics_on_set(
    ...
    for test_inputs, test_labels in test_loader:
        ...
        try:
            test_out = model(test_inputs).squeeze(2)
        except:
            test_out = model(test_inputs).squeeze(1)            
...
#END OF CODE FROM TCPDBench

def get_cp_idx(pred, threshold=None):
    cp = []
    if threshold:
        change_idx = np.where((pred > threshold) == True)[0]
    else:
        change_idx = np.where((pred > pred[0]) == True)[0]        
    if len(change_idx) == 0:
        return cp
    
    cp.append(change_idx[0])
    return cp
...
def cover(model, test_dataloader, threshold):
    ...
    for inputs, labels in test_dataloader:
        ...
        try:
            pred = pred.squeeze(2).cpu().detach().numpy()
        except:
            pred = pred.squeeze(1).cpu().detach().numpy()                
        labels = labels.detach().numpy()

        for i in range(len(pred)):
            p = get_cp_idx(pred[i], threshold)
            l = get_cp_idx(labels[i])
            if len(l) != 0 :
                if len(p) != 0:
                    Sprime = partition_from_cps(p, seq_len)
                    S = partition_from_cps(l, seq_len)
                    cs.append(cover_single(Sprime, S))
                else:
...
def F1_score_ruptures(model, test_dataloader, threshold, margin=10):
    ...
    for inputs, labels in test_dataloader:
        ...
        for i in range(len(pred)):
            ...
            if len(l) != 1:
                if len(p) != 1:
                    precision, recall = precision_recall(l, p)
