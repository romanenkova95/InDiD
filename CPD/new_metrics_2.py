...
def F1_pipeline(train_model, thresholds, device ):
    cover_dict = {}
    f1_dict = {}
    ...
    return cover_dict, f1_dict
             
def calculate_rupture_metric(real_change_ind, predicted_change_ind, seq_len, mode='randindex'):
    metrics = []
    for real, pred in zip(real_change_ind, predicted_change_ind):
        true_partition = [real, seq_len]
        pred_partition = [pred, seq_len]
        if mode == 'randindex':
            metric = randindex(pred_partition, true_partition)
        elif mode == 'hausdorff':
            metric = hausdorff(pred_partition, true_partition)
        #print(pred_partition, true_partition)
        metrics.append(metric)
    return metrics

def calculate_rupture_f1(real_change_ind, predicted_change_ind, seq_len):
    recalls = []
    precisions = []
    for real, pred in zip(real_change_ind, predicted_change_ind):
        true_partition = [real, seq_len]
        pred_partition = [pred, seq_len]
        precision, recall = precision_recall(true_partition, pred_partition)
    ...
    macro_f1_score = macro_precision * macro_recall / (macro_precision + macro_recall)
    return macro_f1_score

def rupture_pipeline(train_model, device='cuda'):
    ...
    with torch.no_grad():
        ...
        for test_inputs, test_labels in train_model.test_dataloader():
            test_out, test_labels = get_models_predictions(
                                        inputs=test_inputs, labels=test_labels, model=train_model, device=device)
            ...
            for threshold in [0.2, 0.5, 0.7]:
                true_labels, predictions = test_labels, test_out > threshold
                mask_real = ~true_labels.eq(true_labels[:, 0][0])
                mask_predicted = ~predictions.int().eq(true_labels[:, 0][0])
                seq_len = true_labels.shape[1]

                real_change_ind = find_first_change(mask_real)
                predicted_change_ind = find_first_change(mask_predicted)

                #covers[threshold].extend(calculate_cover(real_change_ind, predicted_change_ind, seq_len ))
                ris[threshold].extend(calculate_rupture_metric(real_change_ind.cpu(), predicted_change_ind.cpu(), seq_len ))
                hs[threshold].extend(calculate_rupture_metric(real_change_ind.cpu(), predicted_change_ind.cpu(), seq_len, mode='hausdorff' ))
                f1rs[threshold].append(calculate_rupture_f1(real_change_ind, predicted_change_ind, seq_len))
    return  ris, hs, f1rs