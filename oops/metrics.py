...
def get_pareto_metrics_for_threshold(model, test_loader, batch_size, seq_len, threshold_list, device):
    fp_number_list = []
    fn_number_list = []
    delay_list = []
    fp_delay_list = []
    for threshold in threshold_list:
        tp, tn, fp_number, fn_number, mean_delay, mean_fp_delay = evaluate_metrics_on_set(model, test_loader,
                                                                                          batch_size, seq_len, threshold, device)
...
def metrics_by_fp_delay_x_coord(model_name, model, test_loader, batch_size, seq_len, fp_delay, threshold_list, x_coord_arr):
    ...
    for x_coord in range(len(x_coord_arr)):
        ...
        false_positive, delay, false_positive_delay = evaluate_metrics_on_set(model, test_loader, batch_size, seq_len, threshold)
